...
inline std::array<float, 3> materialColor(MaterialType m) {
  ...
  case MaterialType::Water:
    return {0.22f, 0.47f, 0.82f};
  case MaterialType::Soil:
    return {0.20f, 0.60f, 0.25f};
  case MaterialType::Sand:
    return {0.85f, 0.60f, 0.15f};
  case MaterialType::Rock:
    return {0.72f, 0.18f, 0.18f};
  }
  return {1, 1, 1};
}
...
inline MaterialParams defaultMaterialParams(MaterialType m) {
  ...
  case MaterialType::Rock:
    ...
    p.model = ConstitutiveModel::FixedCorotated;
    p.density0 = 2500.f;
    p.youngs_modulus = 5e7f;
    p.poisson_ratio = 0.224f;
    break;
  }
  p.computeDerived();
  return p;
...
// ─────────────────────────────────────────────────────────────────────────────
inline void svd2x2(const Eigen::Matrix2f &A, Eigen::Matrix2f &U,
                   Eigen::Vector2f &sigma, Eigen::Matrix2f &V) {
  Eigen::JacobiSVD<Eigen::Matrix2f> svd(A, Eigen::ComputeFullU |
                                               Eigen::ComputeFullV);
  U = svd.matrixU();
  V = svd.matrixV();
...
// ─────────────────────────────────────────────────────────────────────────────
inline void polarDecompose2x2(const Eigen::Matrix2f &F, Eigen::Matrix2f &R,
                              Eigen::Matrix2f &S) {
  Eigen::Matrix2f U, V;
...
struct SimParams {
  ...
  // Unlike defaultMaterialParams(), these live in the simulation state and
  // can be changed from the UI without recompiling. Existing particles read
  // the current values every time stress is evaluated, so constitutive-model
  // changes take effect immediately; after density changes, press Reset so
  // particle masses are rebuilt from the new density values.
  MaterialParams water_params = defaultMaterialParams(MaterialType::Water);
  MaterialParams soil_params = defaultMaterialParams(MaterialType::Soil);
  MaterialParams sand_params = defaultMaterialParams(MaterialType::Sand);
  MaterialParams rock_params = defaultMaterialParams(MaterialType::Rock);

  void computeDerived() {
    dx = domain_w / grid_nx;
    D_inv = 4.f / (dx * dx);
    water_params.computeDerived();
    soil_params.computeDerived();
    sand_params.computeDerived();
    rock_params.computeDerived();
  }
};
